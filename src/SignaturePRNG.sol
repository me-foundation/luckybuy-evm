...
contract SignaturePRNG is CRC32 {
    ...
    uint256 constant BITS_NEEDED = 14;
    uint256 constant BIT_WINDOW = 32 - BITS_NEEDED;
    uint256 constant BIT_MASK = (1 << BITS_NEEDED) - 1;
    ...
    function _rng(bytes calldata signature) internal view returns (uint32) {
        ...
        return uint32(checksum % BASE_POINTS); // Conversion must be explicit here even though checksum is uint32
    }
}
